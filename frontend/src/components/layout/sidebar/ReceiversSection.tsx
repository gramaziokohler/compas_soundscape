...
export function ReceiversSection({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Number of grid points slider */}
      <div>
        <label className="text-xs mb-2 block" style={{ color: UI_COLORS.NEUTRAL_500 }}>
          Grid resolution (m): <span style={{ color: UI_COLORS.PRIMARY, fontWeight: 'bold' }}>{config.gridResolution}</span>
        </label>
        <input
          type="range"
          value={config.gridResolution}
          onChange={(e) => setConfig(prev => ({ ...prev, gridResolution: parseInt(e.target.value) }))}
          min="1"
          max="30"
