...
import { AUDIO_PLAYBACK, AUDIO_TIMELINE, DEFAULT_SPL_DB, DEFAULT_MAXIMUM_FOLEY_SOUNDS, TTS_DEFAULT_LANGUAGE } from '@/utils/constants';
    ...
    devtools(
      (set, get) => ({
        ...
        syncGeneratedSounds: (sounds) => {
          console.log(
            '[duration-trace][audioControlsStore.syncGeneratedSounds] incoming sounds durations:',
            sounds.map((s: any) => ({ id: s.id, prompt_index: s.prompt_index, copy_index: s.copy_index, duration: s.duration })),
          );
        ...
        setSoundBufferDuration: (soundId, durationSec) => {
          console.log(
            `[duration-trace][audioControlsStore.setSoundBufferDuration] soundId=${soundId} realBufferDurationSec=${durationSec.toFixed(3)} ` +
            `(existing s.duration for this id wins over this value unless it was null/undefined — see bakeOrchestrateSchedule rawDur computation)`,
          );
          set(
        ...
        bakeOrchestrateSchedule: () => {
          ...
          _soundConfigs.forEach((config, configIndex) => {
            ...
            generatedForConfig.forEach((s: any) => {
              ...
              console.log(
                `[duration-trace][bakeOrchestrateSchedule] entry=${meta.entryId} soundId=${s.id} ` +
                `s.duration=${s.duration} soundBufferDurations[id]=${soundBufferDurations[s.id]} → rawDur=${rawDur}`,
              );
              if (rawDur <= 0) return;
              ...
              if (trim) {
                ...
                console.log(
                  `[duration-trace][bakeOrchestrateSchedule] entry=${meta.entryId} soundId=${s.id} ` +
                  `trim applied: trim=${JSON.stringify(trim)} rawDur=${rawDur} → effectiveDur=${effectiveDur}`,
                );
              }
            ...
            if (variantDurations[0] === null) {
              ...
              console.log(
                `[duration-trace][bakeOrchestrateSchedule] entry=${meta.entryId} NO generated sound found yet — ` +
                `falling back to THEORETICAL duration=${theoretical} (config.duration=${(config as any).duration})`,
              );
            ...
            console.log(
              `[duration-trace][bakeOrchestrateSchedule] entry=${meta.entryId} FINAL variantDurations=`,
              variantDurations,
            );
          ...
          console.log(
            `[duration-trace][bakeOrchestrateSchedule] allDurationsKnown=${allDurationsKnown}` +
            (blockingEntries.length > 0 ? ` blocked by: ${blockingEntries.join(', ')}` : ''),
          );
            ...
            entryMap.forEach((entry) => {
              ...
              expressions.forEach((expr, i) => {
                ...
                if (op === 'after') {
                  ...
                  console.log(
                    `[duration-trace][bakeOrchestrateSchedule][normal-pass] ${meta.entryId}[${i}] = after(${refEntryId}[${refIterIdx}]) ` +
                    `refStart=${refStart} + refDuration=${refDuration} (refVariantIdx=${refVariantIdx}) + delay=${delays[i] ?? 0} = ${timestamps[i]}`,
                  );
                  changed = true;
                  ...
                  console.log(
                    `[duration-trace][bakeOrchestrateSchedule][normal-pass] ${meta.entryId}[${i}] = alignEnd(${refEntryId}[${refIterIdx}]) ` +
                    `refStart=${refStart} - thisDuration=${thisDuration} (thisVariantIdx=${thisVariantIdx}) + delay=${delays[i] ?? 0} = ${timestamps[i]}`,
                  );
                  changed = true;
                }
              });
            ...
            if (backEdgeCuts.size > 0) {
              ...
              backEdgeCuts.forEach((_anchorKey, cutKey) => {
                ...
                const anchorStart = anchorEntry?.timestamps[anchorIi]
                  ...
                  ?? 0;
                console.log(
                  `[duration-trace][bakeOrchestrateSchedule][DFS-cut] cutKey=${cutKey} anchorKey=${_anchorKey} ` +
                  `anchorEntry.timestamps[anchorIi]=${anchorEntry?.timestamps[anchorIi]} resolvedFrontier=${resolvedFrontier} ` +
                  `nominalFallback=${parseMMSS(anchorEntry?.meta.timestamps?.[anchorIi])} → anchorStart=${anchorStart}`,
                );
                ...
                if (theoretical !== undefined) {
                  ...
                  console.log(
                    `[duration-trace][bakeOrchestrateSchedule][DFS-cut] APPLIED theoretical anchor for ${cutKey} = ${theoretical}`,
                  );
                } else {
                  ...
                  console.log(
                    `[duration-trace][bakeOrchestrateSchedule][DFS-cut] forward-pass failed for ${cutKey} — ` +
                    `absolute last resort maxT+5 = ${entry.timestamps[ii]}`,
                  );
                }
              });
          ...
          // ── Final summary: entryId → resolved timestamps + durations ──────────
          {
            ...
            console.log('[duration-trace][bakeOrchestrateSchedule] FINAL SUMMARY (entryId → timestamps / variantDurations):', summary);
          }
